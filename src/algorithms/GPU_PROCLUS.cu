...
//  NEW STREAMING-L 
// ============================================================================
// STREAMING-L FIT: No L (n×k) and no dist_n_k (n×k)
// Computes:
//   - d_delta[i]        : min L2 distance from medoid i to any other medoid
//   - d_L_sizes[i]      : |{p : dist(p, m_i) <= d_delta[i]}|
//   - d_sum_abs[i*d+j]  : Σ_{p∈L_i} |x_{p,j} - x_{m_i,j}|
//   - d_X[i*d+j]        : d_sum_abs / d_L_sizes  (finalized later)
// Then reuses your existing Z & D kernels to produce d_Z and d_D.
// ============================================================================

__global__
void compute_delta_from_medoids(float * __restrict__ d_delta,
                                ...
                                int k, int d)
{
    ...
    int mi = d_M_current[i];
    float best = 1.0e30f;

    // For each other medoid j, compute L2 distance between medoid vectors
    for (int j = 0; j < k; ++j) {
        if (j == i) continue;
        int mj = d_M_current[j];

        // Accumulate squared L2 over d dims
        float sum = 0.f;
        for (int t = 0; t < d; ++t) {
            float a = d_data[mi * d + t];
            float b = d_data[mj * d + t];
            float diff = a - b;
            sum += diff * diff;
        }
...
__global__
void streaming_L_accumulate(const float * __restrict__ d_data,
                            const int   * __restrict__ d_M_current,
                            const float * __restrict__ d_delta,
                            float * __restrict__ d_sum_abs,  // (k*d) zeros in
                            int   * __restrict__ d_L_sizes,  // (k)    zeros in
                            int n, int d, int k)
{
    // One thread per point p; stream across all medoids
    int p = blockIdx.x * blockDim.x + threadIdx.x;
    if (p >= n) return;

    // For each medoid, compute L2 distance; if within delta[i], accumulate
    for (int i = 0; i < k; ++i) {
        int mi = d_M_current[i];

        // L2 distance point p to medoid i
        float sum2 = 0.f;
        for (int t = 0; t < d; ++t) {
            float a = d_data[p * d + t];
            float b = d_data[mi * d + t];
            float diff = a - b;
            sum2 += diff * diff;
        }
        float dist = sqrtf(sum2);

        if (dist <= d_delta[i]) {
            // Count membership
            atomicAdd(&d_L_sizes[i], 1);

            // Accumulate |x_pj - m_ij| across all dims
            for (int t = 0; t < d; ++t) {
                float a = d_data[p * d + t];
                float b = d_data[mi * d + t];
                float ad = fabsf(a - b);
                atomicAdd(&d_sum_abs[i * d + t], ad);
            }
        }
    }
}
...
__global__
void finalize_X_from_sums(float * __restrict__ d_X,                // (k*d)
                          const float * __restrict__ d_sum_abs,     // (k*d)
                          const int   * __restrict__ d_L_sizes,     // (k)
                          int k, int d)
{
    ...
    int L = d_L_sizes[i];
    float s = d_sum_abs[idx];
    d_X[idx] = (L > 0) ? (s / (float)L) : 0.0f;



// ============================================================================
// Host wrapper: streaming-L FIT (no L, no dist_n_k)
// Produces d_delta, d_L_sizes, d_X; then fills d_Z and d_D using your existing kernels
// ============================================================================

void gpu_fit_streaming_L(bool *d_D, float *d_Z, float *d_X,
                         int *d_L_sizes, float *d_delta,
                         int *d_M_current,
                         float *d_data,
                         int n, int d, int k, int l)
{
    // 1) delta from medoid-to-medoid distances (full d, L2)
    {
        dim3 block(256), grid((k + block.x - 1) / block.x);
        compute_delta_from_medoids<<<grid, block>>>(d_delta, d_M_current, d_data, k, d);
        gpuErrchk(cudaPeekAtLastError());
    }

    // 2) zero L_sizes and sum_abs
    int *d_zero_ints = d_L_sizes; // reuse pointer; just memset
    gpuErrchk(cudaMemset(d_zero_ints, 0, k * sizeof(int)));

    // Create / reuse a (k*d) buffer for sum_abs; we use d_Z as temporary if memory is tight,
    // but safer to allocate explicitly if you prefer. Here we allocate explicitly.
    float *d_sum_abs = nullptr;
    gpuErrchk(cudaMalloc(&d_sum_abs, (size_t)k * d * sizeof(float)));
    gpuErrchk(cudaMemset(d_sum_abs, 0, (size_t)k * d * sizeof(float)));

    // 3) stream over points to accumulate counts and per-dim absolute deviations
    {
        dim3 block(256), grid((n + block.x - 1) / block.x);
        streaming_L_accumulate<<<grid, block>>>(d_data, d_M_current, d_delta,
                                                d_sum_abs, d_L_sizes,
                                                n, d, k);
        gpuErrchk(cudaPeekAtLastError());
    }

    // 4) finalize X = sum_abs / L_size (shape k×d)
    {
        int total = k * d;
        dim3 block(256), grid((total + block.x - 1) / block.x);
        finalize_X_from_sums<<<grid, block>>>(d_X, d_sum_abs, d_L_sizes, k, d);
        gpuErrchk(cudaPeekAtLastError());
    }

    // 5) compute Z as in your original (cluster-wise standardization across dims)
    //    Reuses your existing kernel:
    ...
    // 6) compute D by selecting l smallest Z per cluster (same kernel you have)
    {
...
//    cudaProfilerStart();
    ...
    // old float *d_dist_n_k = device_allocate_float_zero(n * k);
    // old int *d_L = device_allocate_int(n * k);
    ...
    while (termination_criterion < termination_rounds) {
        ...
        */
        ...
        // STREAMING-L VERSIOn
        gpu_fit_streaming_L(d_D, d_Z, d_X,
                            d_L_sizes, d_delta,
                            d_M_current,
                            d_data,
                            n, d, k, l);       
